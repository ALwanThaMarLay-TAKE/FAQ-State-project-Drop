...
const FaqSection = () => {
  const {faqData , toggleData} = useFAQ()
...
// const toggleData = (id) => {
// SetFaqData(faqData.map(el => el.id === id ? {...el, isOpen : !el.isOpen} : { ...el , isOpen : false }))
  
// }

  return (
    <div className=' py-9 px-32 mx-auto'>
    {faqData.map( el => (<FaqCart key={el.id} question={el.question} id={el.id} isOpen={el.isOpen}   answer={el.answer} />))}
